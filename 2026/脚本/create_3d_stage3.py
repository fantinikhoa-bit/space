...
class Create3DStage3:
    ...
    def create_3d_plot(self):
        ...
        # 设置标题
        ax.set_title('Stage 3: 3D Pareto Front', fontsize=16, pad=30)
        
        # 添加图例
        ax.legend(loc='upper right', fontsize=12)
        
        # 设置不同的视角，增加立体感
        ax.view_init(elev=25, azim=60)  # 调整视角，从更低更侧的角度查看，增加深度感
        ...
        # 保存图形
        output_file = os.path.join(result_dir, 'Q4_Stage3_3D_Pareto.png')
        plt.savefig(output_file, dpi=300, bbox_inches='tight')
        
        # 显示图形
        plt.show()
        
        print(f"Stage 3 3D Pareto front saved to: {output_file}")
        print(f"\nPlot details:")
        print(f"- Feasible points: {len(feasible_costs)}")
        print(f"- Pareto optimal points: {len(pareto_costs)}")

if __name__ == "__main__":
    creator = Create3DStage3()
